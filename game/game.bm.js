...
const sprites32x32 = {
  ...
  unionjack: [
    "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK",
    "KRRWWBBBBBBBBWWWRRWWBBBBBBBBWWRK",
    "KWRRWWBBBBBBWWWRRRWWBBBBBBWWRRWK",
    "KBWRRWWBBBBWWWRRRRRWWBBBBWWRRWBK",
    "KBBWRRWWBBWWWRRRRRRRWWBBWWRRWBBK",
    "KBBBWRRWWWWRRRRRRRRRRWWWWWRRWBK",
    "KBBBBWRRRRRRRRRRRRRRRRRRRRRWBBBK",
    "KWWWWWRRRRRRRRRRRRRRRRRRRRRWWWWK",
    "KWWWWWRRRRRRRRRRRRRRRRRRRRRWWWWK",
    ...
    "KBBWRRWWBBWWWRRRRRRRWWBBWWRRWBBK",
    "KBWRRWWBBBBWWWRRRRRWWBBBBWWRRWBK",
    "KWRRWWBBBBBBWWWRRWWBBBBBBWWRRWK",
    "KRRWWBBBBBBBBWWWRRWWBBBBBBBBWWRK",
    "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK",
    "................................",
    "................................",
  ...
  jalur: [
    "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK",
    "KBBBBBBBBBBBBBBBBRRRRRRRRRRRRRRK",
    "KBBBBBBBBBBBBBBBBWWWWWWWWWWWWWWK",
    "KBBBBBYYYYYBBBBBBRRRRRRRRRRRRRRK",
    "KBBBBYYYYYYYYBBBBWWWWWWWWWWWWWWK",
    "KBBBYYYBBYYYYYBBBRRRRRRRRRRRRRRK",
    "KBBBYYYBBBYYYYBBBWWWWWWWWWWWWWWK",
    "KBBBYYYBBBBYYYBBBRRRRRRRRRRRRRRK",
    "KBBBBYYYYYYYYBBBBWWWWWWWWWWWWWWK",
    "KBBBBBYYYYYBBBBBBRRRRRRRRRRRRRRK",
    "KBBBBBBBBBBBBBBBBWWWWWWWWWWWWWWK",
    "KRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRK",
  ...
  motorcade: [
    ...
    "KBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBK",
    "KBBBKKKKKBBBBBBBBBBBBBBBKKKKKBBK",
    "KBBKKKKKKKBBBBBBBBBBBBBKKKKKKKBK",
    "KBBKKWKWKKBBBBBBBBBBBBBKKWKWKKBK",
    "KBBKKKKKKKBBBBBBBBBBBBBKKKKKKKBK",
    "KBBBKKKKKBBBBBBBBBBBBBBBKKKKKBBK",
    "KBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBK",
    "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK",
...
function applyReflection(pts, refType, refVal) {
  return pts.map((pt) => {
    ...
    if (refType === "x-axis") {
      ...
      rx = -pt.y;
      ry = -pt.x;
    } else if (refType === "x_line") {
      rx = 2 * refVal - pt.x;
    } else if (refType === "y_line") {
      ry = 2 * refVal - pt.y;
    }
    return { x: rx, y: ry };
  });
...
function applyCombinedStep(pts, step) {
  ...
  if (type === "translation") {
    return applyTranslation(
      pts,
      state[`combo${step}_tx`],
      state[`combo${step}_ty`],
    );
  }
  if (type === "rotation") {
    return applyRotation(
      pts,
      state[`combo${step}_angle`],
      state[`combo${step}_dir`],
      {
        x: state[`combo${step}_rcx`],
        y: state[`combo${step}_rcy`],
      },
    );
  }
  return pts;
...
function draw32x32PixelSprite(ctx, type, box, transform = {}) {
  ...
  if (image && image.complete && image.naturalWidth) {
    ...
    for (const [xOffset, yOffset] of [
      ...
      ctx.drawImage(
        outline,
        -imageWidth / 2 + xOffset * outlineThickness,
        -imageHeight / 2 + yOffset * outlineThickness,
        imageWidth,
        imageHeight,
      );
    }
...
function getTransformedPointsForStage(stg, isTarget = false) {
  ...
  if (isTarget) {
    ...
    if (stg.type === "enlargement" || stg.type === "enlargement_negative") {
      ...
      return applyRotation(pts, state.angle, state.dir, {
        x: state.rcx,
        y: state.rcy,
      });
    } else if (stg.type === "reflection") {
...
function drawCanvas() {
  ...
  for (let i = 0; i <= GRID_UNITS; i++) {
    ctx.beginPath();
    ctx.moveTo(i * unitPixels, 0);
    ctx.lineTo(i * unitPixels, canvas.height);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(0, i * unitPixels);
    ctx.lineTo(canvas.width, i * unitPixels);
    ctx.stroke();
  }
  ctx.restore();
  ...
  ctx.lineTo(originPx.x, canvas.height);
  ctx.stroke();
  ...
  draw32x32PixelSprite(
    ctx,
    stg.objectType,
    targetBox,
    getSpriteTransform(stg, true),
  );
  ...
  if (stg.type === "combined" && state.combo1_type) {
    ...
    draw32x32PixelSprite(
      ...
      stg.objectType,
      stepOneBox,
      getSpriteTransform(stg, false, true),
    );
  }
  ...
  if (stg.type === "enlargement" || stg.type === "enlargement_negative") {
    ...
    if (state.refType === "x-axis") {
      ...
      const p = gridToPixel(state.refVal, 0);
      ctx.beginPath();
      ctx.moveTo(p.x, 0);
      ctx.lineTo(p.x, canvas.height);
      ctx.stroke();
    } else if (state.refType === "y_line") {
      const p = gridToPixel(0, state.refVal);
      ctx.beginPath();
      ctx.moveTo(0, p.y);
      ctx.lineTo(canvas.width, p.y);
      ctx.stroke();
    ...
    const rotationStep =
      state.combo2_type === "rotation"
        ? 2
        : state.combo1_type === "rotation"
          ? 1
          : null;
    if (rotationStep) {
      const cp = gridToPixel(
        state[`combo${rotationStep}_rcx`],
        state[`combo${rotationStep}_rcy`],
      );
      ctx.fillStyle = "#ffdf00";
      ctx.beginPath();
      ctx.arc(cp.x, cp.y, 7, 0, Math.PI * 2);
...
function drawTransformationGuides(stg, originalPoints, transformedPoints) {
  ...
  if (stg.type === "rotation") {
    ...
    const radius = Math.min(
      34,
      Math.hypot(start.x - centerPx.x, start.y - centerPx.y) * 0.35,
    );
    const startAngle = Math.atan2(start.y - centerPx.y, start.x - centerPx.x);
    const canvasAngle =
      (rotationForCanvas(state.angle, state.dir) * Math.PI) / 180;
    ...
    ctx.fillText(
      `${state.angle}°`,
      centerPx.x + Math.cos(labelAngle) * (radius + 13),
      centerPx.y + Math.sin(labelAngle) * (radius + 13),
    );
    ctx.restore();
    return;
  }
...
function getSpriteTransform(stg, isTarget = false, stepOneOnly = false) {
  ...
  if (isTarget)
    return {
      rotationDegrees: rotationForCanvas(stg.targetAngle, stg.targetDir),
    };
  let rotation = 0;
  if (state.combo1_type === "rotation")
    rotation += rotationForCanvas(state.combo1_angle, state.combo1_dir);
  if (!stepOneOnly && state.combo2_type === "rotation")
    rotation += rotationForCanvas(state.combo2_angle, state.combo2_dir);
...
function attachInputListeners() {
  ...
  if (stg.type === "enlargement" || stg.type === "enlargement_negative") {
    document.getElementById("scale-input").addEventListener("input", (e) => {
      state.k = parseFloat(e.target.value) || 0;
      drawCanvas();
    });
    document.getElementById("cx-input").addEventListener("input", (e) => {
      state.cx = parseInt(e.target.value) || 0;
      drawCanvas();
    });
    document.getElementById("cy-input").addEventListener("input", (e) => {
      state.cy = parseInt(e.target.value) || 0;
      drawCanvas();
    });
  } else if (stg.type === "rotation") {
    ...
    document.getElementById("rcx-input").addEventListener("input", (e) => {
      state.rcx = parseInt(e.target.value) || 0;
      drawCanvas();
    });
    document.getElementById("rcy-input").addEventListener("input", (e) => {
      state.rcy = parseInt(e.target.value) || 0;
      drawCanvas();
    });
  } else if (stg.type === "reflection") {
    document
      .getElementById("ref-type-select")
      .addEventListener("change", (e) => {
        state.refType = e.target.value;
        drawCanvas();
      });
    document.getElementById("ref-val-input").addEventListener("input", (e) => {
      state.refVal = parseInt(e.target.value) || 0;
      drawCanvas();
    });
  } else if (stg.type === "translation") {
    document.getElementById("tx-input").addEventListener("input", (e) => {
      state.tx = parseInt(e.target.value) || 0;
      drawCanvas();
    });
    document.getElementById("ty-input").addEventListener("input", (e) => {
      state.ty = parseInt(e.target.value) || 0;
      drawCanvas();
    });
  } else if (stg.type === "combined") {
    [1, 2].forEach((step) => {
      ...
      const bindNumber = (field, key) => {
        ...
        if (input)
          input.addEventListener("input", (e) => {
            state[`combo${step}_${key}`] = parseInt(e.target.value) || 0;
            drawCanvas();
          });
      };
      bindNumber("tx", "tx");
      ...
      if (angle)
        angle.addEventListener("change", (e) => {
          state[`combo${step}_angle`] = parseInt(e.target.value);
          drawCanvas();
        });
      const dir = document.getElementById(`${prefix}-dir`);
      if (dir)
        dir.addEventListener("change", (e) => {
          state[`combo${step}_dir`] = e.target.value;
          drawCanvas();
        });
    });
  }
...
function checkAnswer() {
  ...
  let isCorrect = !(
    stg.type === "combined" &&
    (state.combo1_type !== "translation" || state.combo2_type !== "rotation")
  );
  for (let i = 0; i < targetPts.length; i++) {
    if (
      Math.abs(targetPts[i].x - userPts[i].x) > 0.1 ||
      Math.abs(targetPts[i].y - userPts[i].y) > 0.1
    ) {
      isCorrect = false;
      break;
    }
...
function loadStage(idx) {
  ...
  document.getElementById("level-num").innerText =
    `${stg.stage} / ${stages.length}`;
  document.getElementById("stage-title-text").innerText = stg.title;
  document.getElementById("historical-fact-text").innerText =
    stg.historicalFact;
